...
func init() {
	...
	// 群聊并且艾特机器人
	engine.OnMessage(robot.AtMeOrReference).SetBlock(true).SetPriority(9999).Handle(func(ctx *robot.Ctx) {
		if ctx.Event.Type == robot.EventPrivateChat {
			//ctx.ReplyText("这是一个自动回复。")
			return
		}
		//if ctx.Event.Type == robot.EventGroupChat {
		//	//ctx.ReplyText("这是一个自动回复。")
		//	return
		//}
		if !ctx.IsAt() {
		...
		var (
			now = time.Now().Local()
			msg = ctx.MessageString()[len("@一个没有感情的机器人"):]

			chatRoom = GlmChatRoom{
		...
		if ctx.IsReference() {
			...
			if c, ok := chatRoomCtx.Load(chatRoom.chatId); ok {
				// 判断距离上次聊天是否超过10分钟了
				if now.Sub(c.(GlmChatRoom).chatTime) > 10*time.Minute {
					chatRoomCtx.LoadAndDelete(chatRoom.chatId)
					chatRoom.content = *initMessage(msg)
				} else {
					chatRoom.content = append(c.(GlmChatRoom).content, Message{Role: "user", Content: msg})
				}
			} else {
				chatRoom.content = *initMessage(msg)
			}
		...
		// 截取最后7条对话
		var le = 7
		if len(chatRoom.content) > le {
			chatRoom.content = chatRoom.content[len(chatRoom.content)-le:]
		}
		answer := sendArr(chatRoom.content)
		chatRoom.content = append(chatRoom.content, Message{Role: "assistant", Content: answer})
		chatRoomCtx.Store(chatRoom.chatId, chatRoom)
		//answer = replaceSensitiveWords(answer)
		err := ctx.ReplyTextAndAt(answer)
		// 将结果放到匹配队列，触发其它插件
		if err == nil {
			return
		}
...
//api := "https://api.vvhan.com/api/love?type=json"
//if err := req.C().Get(api).Do().Into(&data); err != nil {
//	log.Errorf("获取失败: %v", err)
//	return
//}
//err := ctx.ReplyTextAndAt(data.Ishan)
//// 将结果放到匹配队列，触发其它插件
//if err == nil {
//	return
//}
